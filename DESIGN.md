# 随心记 iOS 设计规范

## 体验

打开即写。主页保留空白编辑器、自动键盘和一键工作流；整理、回顾和配置使用系统导航及表单。界面服务于记录内容。

## 颜色和材质

- 普通按钮、导航和菜单图标使用 `Design.controlColor`（`UIColor.label`），浅色模式为黑色，深色模式随系统适配。
- 强调色：`UIColor.systemIndigo`，仅用于标签、选中状态等需要强调的信息，不作为全局控件 tint。
- 专注模式工作流按钮沿用黑底白字，包括降低透明度时的不透明背景。
- 编辑和会话背景：`systemBackground`；列表：`systemGroupedBackground`、`secondarySystemGroupedBackground`。
- 正文：`Color.primary` / `UIColor.label`；说明：`Color.secondary` / `secondaryLabel`。
- 删除和失败使用系统 red，并同时提供名称、符号或文字状态。
- Liquid Glass 限于底部操作区域；表单和消息使用内容层背景。降低透明度时，操作区域使用不透明系统背景。

## 字体和尺寸

- 正文使用 `.body`，说明使用 `.subheadline` / `.footnote`，日期等元数据使用 `.caption`。
- 主页名称使用已确认的宋体图片 `AppTitleWordmark`，左对齐、默认高 34 pt，随 Dynamic Type 缩放并限制在导航栏可用高度内；正文保持 `.body`（默认 17 pt）。原图保存在资源目录中，显示时裁去外围留白并用字形蒙版跟随 `.primary` 适配浅深色，读屏名称保留“随心记”和标题语义。
- UIKit 编辑器使用 `UIFont.preferredFont(forTextStyle: .body)` 和 Dynamic Type。
- 主页标题下方为整个输入区域增加 12 pt 留白，占位文字与光标同步下移；普通与专注模式保持相同留白。
- 自定义操作热区至少 44 × 44 pt；图标选择单元随字体缩放。
- 正文不限制辅助功能字号。紧凑工具栏的符号保持 20 pt，以避免图标侵占正文；最大字号下工作流转为系统菜单。
- 长表单、权限详情支持滚动。统计日历在辅助功能字号或不足 308 pt 的内容宽度下采用日期列表。
- 主编辑器和 AI 会话的阅读宽度上限为 720 pt。

## 控件与状态

- 导航：NavigationStack；工作流在常规宽度沿用 NavigationSplitView。
- “随心记”属于主页内容，通过页面覆盖层对齐原导航栏左上角的视觉位置，不额外占用一行，也不使用 ToolbarItem，避免与二级页返回按钮联动；顶部更多菜单和 NavigationStack 保持原生导航结构。底部叠页图标统一作为记录与搜索入口，草稿为空时打开列表，有内容时按草稿搜索。
- 从记录或 AI 助手返回主页时，原生返回按钮在转场开始时隐藏，避免玻璃背景覆盖主页标题；取消侧滑时恢复按钮，导航和返回手势仍由 NavigationStack 管理。
- 主页底部按功能保留三组：工作流、标签与记录、清除／恢复。组间距 12 pt、组内间距 4 pt；图标在 label 内统一指定 20 pt，独立图标按钮的点击区域为 48 × 48 pt。标签数量承担状态提示，图标保持轮廓样式。
- 普通与专注模式的底部按钮及操作行统一高 48 pt；专注按钮仅横向展开并显示工作流名称，切换时保持右侧清除／恢复按钮的垂直位置。
- 进入或退出专注模式时，主页标题、顶部导航与编辑器顶部留白保持稳定；界面切换动画只作用于底部按钮。
- 专注模式面向熟练用户，底部不显示执行进度、完成、清除和发送等文字提示；执行状态通过按钮加载反馈表达，错误使用原生弹窗。
- 草稿编辑器使用原生 UITextView 光标，由系统管理显示和闪烁；专注模式切换不额外冻结、延迟恢复或重置光标动画。
- 普通与专注模式共用同一块工作流玻璃胶囊，宽度通过轻微弹簧连续变化，内部内容使用系统模糊替换过渡；标签与退出专注共用固定位置的按钮，通过系统符号替换切换图标，避免交叉淡入淡出两套背景。
- 列表：iPhone 使用 insetGrouped，保留系统分隔、重排、侧滑与选择惯例。
- 多选：勾号表示全部选中，减号表示部分选中；读屏名称同时包含状态。
- 表单：字段名称常驻；AI 密钥关闭自动大写与纠错；主机和端口在保存前校验。
- 有编辑项的弹窗提供“取消／保存”；只读详情提供“完成”；不可逆删除与配置覆盖使用明确确认。
- 清除草稿显示恢复入口；复制和执行结果提供文字或辅助功能反馈。
- 系统减少动态效果设置禁用自定义动画。优先采用系统按钮和原生弹窗的可访问行为。

逐页修改清单与实测范围见 [HIG 审视记录](docs/ios-hig-audit.md)。

## 文案

- 输入中的内容称“草稿”，列表内容称“记录”，AI 交互称“对话”。使用“工作流／步骤”，保留用户自定义名称。
- CTA 使用动作与对象，如“导出全部记录”“保存步骤”“保留当前配置”。
- 首次使用说明仅在普通模式的空白编辑页内展示，开始输入或进入专注模式后收起。空状态区分首次使用、搜索无结果与环境不可用，并提供下一步。
- 错误提示说明问题和可执行的恢复办法；AI 与网络错误通过 UserFacingError 统一处理。主动停止采用中性状态。
- 不把发送指令写成接收端已粘贴，也不把本机保存写成 iCloud 已同步；删除、配置覆盖与设备授权说明具体范围。
- 完整文案对比与验证见 [UX Writing 审视记录](docs/ux-writing-audit.md)。
