// Derived from OpenMinis (https://github.com/OpenMinis/OpenMinis) — Agent/ToolLoopDetector.swift
// Copyright (C) OpenMinis contributors. Licensed under GPL-3.0; see LICENSE.
// Modifications for vibetaking: removed shell polling tool names from isPollTool.

import Foundation
import CryptoKit

private nonisolated let logger = AppLogger(category: "ToolLoopDetector")

// MARK: - Public types

nonisolated struct ToolCallRecord: Sendable {
    let toolName: String
    let argsHash: String
...
nonisolated enum LoopLevel: Equatable, Sendable {
    case none
    case warning
    case critical
}

nonisolated struct LoopCheckResult: Equatable, Sendable {
    let level: LoopLevel
    let message: String?
...
nonisolated struct ToolLoopConfig: Sendable {
    var historySize: Int
...
nonisolated final class ToolLoopDetector: @unchecked Sendable {
    private let config: ToolLoopConfig
    private var history: [ToolCallRecord] = []
    /// Map<warningKey, lastBucket> — throttles repeated warnings to once per N triggers.
    ...
    func check(toolName: String, params: [String: Any]) -> LoopCheckResult {
        ...
        // Strategy 2: global_circuit_breaker — backstop.
        ...
        // Strategy 3: generic_repeat — total occurrences (not necessarily contiguous).
    ...
    func record(
        ...
        // Re-evaluate post-hoc: if generic_repeat threshold is now hit,
    ...
    /// UI-only fields that the model freely varies and which must NOT
    /// contribute to the args hash.
    ...
    /// Strip volatile fields (timestamps, request IDs, durations) before
    /// hashing so genuinely identical outcomes collapse to one hash.
