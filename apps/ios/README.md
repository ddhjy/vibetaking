...
设置页底部默认仅显示产品版本号（如 `1.0.2`）。连点版本号 5 次，会显示「调试」分组，在「打开 FLEX」上方展示「构建号」（如 `5`）；构建号缺失或为空时省略该行。

「打开 FLEX」用于唤起 [FLEX](https://github.com/FLEXTool/FLEX) 悬浮调试工具条，该按钮只在 Debug 构建中可用。
