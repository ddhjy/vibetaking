...
一款基于 SwiftUI 的 iOS 速记应用，主打“打开即写”：启动后不先展示笔记列表，而是直接进入空白编辑页，自动聚焦输入区、唤起键盘。草稿实时保存，记录以 Markdown 文件落盘，并可通过 iCloud Drive 在设备间同步。
...
vibetaking 的核心是让用户以尽可能低的成本完成一次快速记录。很多笔记产品以回顾为中心，打开后首先看到的是列表；这里把入口放回记录本身：一张空白页、一个已经就绪的光标和键盘。空白页减少选择和干扰，也会自然产生“把它填上”的输入动机。

这种“快”来自几个取舍：

- **入口快**：主页就是编辑页，不需要先新建、选择分类或进入某篇笔记。
- **启动快**：界面使用原生组件构建，优先保证启动、动画和交互的响应速度。
- **输入快**：记录以纯文本为核心，不区分标题和正文，也不提供复杂排版，用户只需要关心写下内容。
- **整理快**：信息组织只保留记录和标签；随着标签积累，零散文本会逐渐变得可分类、可检索、可回顾。

需要更快输入时，可以配合系统键盘听写，或借助豆包等语音转文字工具。语音不是应用内置能力，而是输入方式的一种延伸。

## 核心能力

- **打开即写**：进入主页后自动聚焦全屏输入框；草稿写入 `_draft.md`，退出或切走应用也不会丢。
- **纯文本记录**：不区分标题与正文，不在格式上打断输入节奏。
