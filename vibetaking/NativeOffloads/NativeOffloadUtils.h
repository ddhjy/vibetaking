...
//  Modifications for vibetaking: removed iSH guest-filesystem stub creation
//  (kernel/fs.h dependencies); noff_resolve_host_path now resolves against a
//  storage root registered via noff_set_storage_root().
//
...
NSString *_Nullable noff_find_arg(int argc, char *_Nonnull *_Nonnull argv, const char *name);

/// Check if a flag is present (e.g. --compact, -q, --help).
BOOL noff_has_flag(int argc, char *_Nonnull *_Nonnull argv, const char *name);

/// Get the subcommand (first non-flag argument after argv[0]).
/// Returns nil if no subcommand found.
NSString *_Nullable noff_get_subcommand(int argc, char *_Nonnull *_Nonnull argv);

/// Collect all positional arguments (non-flag, non-option-value args after subcommand).
NSArray<NSString *> *noff_positional_args(int argc, char *_Nonnull *_Nonnull argv);
...
/// Register the storage root used by noff_resolve_host_path. Call once from
/// the app before invoking any offload handler that writes files.
void noff_set_storage_root(NSString *rootPath);

/// Convert a tool-provided path to a host path inside the registered storage
/// root (leading "/" stripped, ".." rejected).
/// Returns nil if the path is empty, escapes the root, or no root registered.
