...
//  Modifications for vibetaking: removed iSH kernel includes and guest stub
//  creation; noff_resolve_host_path resolves against a registered storage root.
//
...
static NSString *_noff_storage_root = nil;

void noff_set_storage_root(NSString *rootPath) {
    _noff_storage_root = [rootPath copy];
}

NSString *_Nullable noff_resolve_host_path(NSString *guestPath) {
    if (guestPath.length == 0) return nil;
    if (_noff_storage_root.length == 0) return nil;

    // Strip leading "/" and reject traversal outside the storage root.
    NSString *relative = guestPath;
    while ([relative hasPrefix:@"/"]) {
        relative = [relative substringFromIndex:1];
    }
    NSString *candidate = [[_noff_storage_root stringByAppendingPathComponent:relative] stringByStandardizingPath];
    NSString *root = [_noff_storage_root stringByStandardizingPath];
